...
Ici on souhaite ne conserver que les pêches réalisées dans le cadre des réseaux, on utilise la fonction `mef_ajouter_objectif()` puis on filtre sur les modalités à conserver : 
...
On pourrait aussi restreindre le jeu de données par exemple aux pêches complètes et réalisées sans bateau avec les fonctions `mef_ajouter_type_protocole()` et `mef_ajouter_moyen_prospection()` (code non exécuté ici).
...
Dans ces cas, pour éviter les biais d'échantillonnage dans les tendances, on filtre le tableau de données pour ne retenir qu'une des opération. Retenons par exemple la dernière de l'année (pêche d'automne). Comme il y a même  quelques cas où plusieurs opérations sont enregistrées le même jour, on n'en retient qu'une.
...
depts <- tod::osm_depts_tod(url = url,
                            repertoire = "../raw_data",
                            crs_sortie = 2154)
```
...
regions <- tod::osm_depts_tod(url = url,
                            repertoire = "../raw_data",
                            crs_sortie = 2154)
