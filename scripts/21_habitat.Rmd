...
```{r}
moy1 <- captures_et_env %>% 
  group_by(esp_code_alternatif, parametre, presence) %>% 
    summarise(moyenne1 = mean(valeur_parametre, na.rm = T),
              n1 = n()) %>% 
  ungroup()

moy2 <- captures_et_env2 %>% 
  group_by(esp_code_alternatif, parametre, presence) %>% 
    summarise(moyenne2 = mean(valeur_parametre, na.rm = T),
              n2 = n()) %>% 
  ungroup()

moy <- moy1 %>% 
  left_join(moy2) %>% 
  mutate(delta_abs = moyenne1 - moyenne2,
         delta_rel = (moyenne1 - moyenne2) / moyenne1)
```
...
```{r}
moy3 <- captures_et_env3 %>% 
  group_by(esp_code_alternatif, parametre, presence) %>% 
    summarise(moyenne3 = mean(valeur_parametre, na.rm = T),
              n3 = n()) %>% 
  ungroup()
...
```{r, echo = FALSE}
save(captures_et_env, captures_et_env3, file = "../processed_data/captures_et_env.RData")
```
