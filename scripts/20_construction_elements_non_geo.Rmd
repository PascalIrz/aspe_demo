...
captures_et_env_nested <- captures_et_env %>%
  group_by(esp_code_alternatif, presence, parametre) %>% 
  nest() %>% 
  ungroup() %>% 
    left_join(y = occurrences %>% 
              select(esp_code_alternatif, parametre, presences),
            by = c("esp_code_alternatif", "parametre"))

set.seed = 123

# sous-échantillonnage
captures_et_env_ssech <- captures_et_env_nested %>%
  mutate(ssech = map2(.x = data,
                      .y = presences,
                      .f = sample_n,
                      replace = TRUE))

# assemblage
captures_et_env <- captures_et_env_ssech %>% 
  select(-data) %>%
  unnest(ssech)

# allègement du dataframe
captures_et_env <- captures_et_env %>% 
  select(-presences, -dens_ind_1000m2, -effectif) %>% 
...
captures_et_env %>% 
  ...
  ggplot(aes(x = valeur_parametre, fill = presence)) +
...
```{r}
esp_communes <- occurrences %>% 
  filter(presences > 1000) %>% 
  pull(esp_code_alternatif) %>% 
  unique %>% 
  as.character
mon_parametre <- "temp_juillet"
mon_parametre <- "distance_mer"
...
captures_et_env %>% 
  ...
  ggplot(aes(x = annee, y = valeur_parametre, col = presence)) +
    ...
    labs(x = "", y = mon_parametre, title = mon_espece) +
    scale_x_continuous(limits = c(1995, NA)) +
    facet_wrap(~esp_code_alternatif, scales = "free_y")
```
...
env %>% 
  ggplot(aes(x = as.factor(annee), y = valeur_parametre)) +
